...
if ( ! function_exists( 'pixassist_get_recipes_copy' ) ) {
	...
	function pixassist_get_recipes_copy() {
		return array(
			'title'          => esc_html__( 'Recipes', 'pixelgrade-assistant' ),
			'description'    => esc_html__( 'Apply a complete starter recipe as one bundle, then fine-tune individual layouts afterward.', 'pixelgrade-assistant' ),
			'loadLabel'      => esc_html__( 'Load recipes', 'pixelgrade-assistant' ),
			'loading'        => esc_html__( 'Loading recipes...', 'pixelgrade-assistant' ),
			'empty'          => esc_html__( 'No recipes are available yet.', 'pixelgrade-assistant' ),
			'failure'        => esc_html__( 'Recipes could not be loaded. Please try again.', 'pixelgrade-assistant' ),
			'partialFailure' => esc_html__( 'Some recipe sources could not be loaded.', 'pixelgrade-assistant' ),
			'applyLabel'     => esc_html__( 'Apply', 'pixelgrade-assistant' ),
			'applying'       => esc_html__( 'Applying recipe...', 'pixelgrade-assistant' ),
			'applySuccess'   => esc_html__( 'Recipe applied.', 'pixelgrade-assistant' ),
			'applyFailure'   => esc_html__( 'Recipe could not be applied. Please try again.', 'pixelgrade-assistant' ),
			'appliedLabel'   => esc_html__( 'Applied', 'pixelgrade-assistant' ),
			'removeLabel'    => esc_html__( 'Remove', 'pixelgrade-assistant' ),
			'removing'       => esc_html__( 'Removing recipe...', 'pixelgrade-assistant' ),
			'removeSuccess'  => esc_html__( 'Recipe removed.', 'pixelgrade-assistant' ),
			'removeFailure'  => esc_html__( 'Recipe could not be removed. Please try again.', 'pixelgrade-assistant' ),
			'lookLabel'      => esc_html__( 'Include look', 'pixelgrade-assistant' ),
			'sampleLabel'    => esc_html__( 'Include sample content', 'pixelgrade-assistant' ),
			'layoutOnly'     => esc_html__( 'Layout only by default', 'pixelgrade-assistant' ),
			'partsLabel'     => esc_html__( 'Parts', 'pixelgrade-assistant' ),
			'templatesLabel' => esc_html__( 'Templates', 'pixelgrade-assistant' ),
			'featuresLabel'  => esc_html__( 'Features', 'pixelgrade-assistant' ),
			'sourceHeading'  => esc_html__( 'Source', 'pixelgrade-assistant' ),
			'premiumLabel'   => esc_html__( 'Plus', 'pixelgrade-assistant' ),
			'deviatedLabel'  => esc_html__( 'Customized after apply', 'pixelgrade-assistant' ),
		);
	}
}
