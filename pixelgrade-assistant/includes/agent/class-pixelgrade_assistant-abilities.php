...
class PixelgradeAssistant_Abilities {
	...
	public static function register_category() {
		...
		wp_register_ability_category(
			self::CATEGORY,
			array(
				'label'       => __( 'Pixelgrade', 'pixelgrade-assistant' ),
				'description' => __( 'Design system, licensing, starter content and block operations for the Pixelgrade stack.', 'pixelgrade-assistant' ),
			)
		);
	}
	...
	private static function permission_callback( $descriptor ) {
		return static function () use ( $descriptor ) {
			...
			if ( ! self::entitled( $descriptor ) ) {
				return new WP_Error(
					'permission_denied',
					__( 'This ability requires a Pixelgrade Plus entitlement that is not currently granted.', 'pixelgrade-assistant' )
				);
			}
			...
			if ( ! current_user_can( PixelgradeAssistant_Agent_Core::CAPABILITY ) ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'This ability requires the "%s" capability.', 'pixelgrade-assistant' ),
	...
	public static function needs_confirmation( $what ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( '%s is destructive. Re-run with "confirm": true once you intend it.', 'pixelgrade-assistant' ),
				$what
			)
		);
	}
	...
	public static function envelope_schema( $data_schema ) {
		return array(
			...
			'properties' => array(
				'ok'       => array(
					'type'        => 'boolean',
					'description' => __( 'Always true here: the call completed. A failure arrives as an error, not as this envelope.', 'pixelgrade-assistant' ),
				),
				'code'     => array(
					'type'        => 'string',
					'description' => __( 'Stable machine token, never translated. "ok" means nothing to inspect; anything else means the call completed WITH findings you must read (e.g. "partial").', 'pixelgrade-assistant' ),
				),
				'summary'  => array(
					'type'        => 'string',
					'description' => __( 'One human-readable line.', 'pixelgrade-assistant' ),
				),
				'data'     => $data_schema,
				'warnings' => array(
					'type'        => 'array',
					'description' => __( 'Findings that did not fail the call. Each has at least {code, message}.', 'pixelgrade-assistant' ),
	...
	public static function descriptors() {
		...
		return array(
				...
				'input_schema' => array(
					...
					'properties' => array(
						'refresh' => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Bypass the cached hub config and fetch a fresh starter descriptor list.', 'pixelgrade-assistant' ),
						),
					),
				...
				'output_schema' => self::envelope_schema(
					array(
						...
						'properties' => array(
							'starters' => array(
								'type'        => 'array',
								'description' => __( 'The normalized hub catalog. Each entry carries the demo key and its source base URL.', 'pixelgrade-assistant' ),
								'items'       => array( 'type' => 'object' ),
							),
						),
					)
			...
			'pixelgrade/import-starter' => array(
				'label'       => __( 'Import a starter site', 'pixelgrade-assistant' ),
				'description' => __( 'Import a starter site\'s full content — posts, pages, media, taxonomies, widgets and design settings — into this site. DESTRUCTIVE and NOT idempotent: it creates content and media every time it runs, and before anything is journaled it force-deletes an untouched default "Hello world!" post and "Sample Page" pair if present; that deletion is not undone by pixelgrade/reset-starter-content. Requires confirm:true. Get demo_key and source_url from pixelgrade/list-starters; source_url must be https and its host must be on the allowlist. A result with code "partial" means some units imported and some failed — read warnings and data before retrying. code "missing_required_plugins" means nothing usable was imported until those plugins are installed. code "not_a_starter" means the key names a curated library rather than a whole site: nothing was touched, and its items are added one at a time with pixelgrade/import-page-pattern.', 'pixelgrade-assistant' ),
				...
				'input_schema' => array(
					...
					'properties' => array(
						'demo_key'   => array(
							'type'        => 'string',
							'description' => __( 'The starter/demo key, as listed by pixelgrade/list-starters.', 'pixelgrade-assistant' ),
						),
						'source_url' => array(
							'type'        => 'string',
							'description' => __( 'The starter\'s source SCE REST base URL (its baseRestUrl). Must use https://.', 'pixelgrade-assistant' ),
						),
						'confirm'    => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Must be true. This import creates content it cannot cleanly reverse.', 'pixelgrade-assistant' ),
						),
					),
				...
				'execute'     => static function ( $input = array() ) {
					$validated = PixelgradeAssistant_Agent_Core::validate_keyed_source(
						...
						__( 'You need to provide demo_key and source_url.', 'pixelgrade-assistant' ),
						__( 'source_url must use https://.', 'pixelgrade-assistant' )
					);
					...
					if ( empty( $input['confirm'] ) ) {
						return self::needs_confirmation( __( 'Importing starter content', 'pixelgrade-assistant' ) );
					}
			...
			'pixelgrade/reset-starter-content' => array(
				'label'       => __( 'Reset starter content', 'pixelgrade-assistant' ),
				'description' => __( 'Undo every starter import journaled on this site: delete the imported posts, media and terms, and restore the options and theme mods that were replaced. DESTRUCTIVE — it deletes content. Requires confirm:true. Idempotent: running it again on an already-reset site is a no-op. code "partial" means some journaled posts were already gone (data.posts_missing counts them); that is a finding, not a failure. It does NOT restore the default "Hello world!" post and "Sample Page" that an import may have removed.', 'pixelgrade-assistant' ),
				...
				'input_schema' => array(
					...
					'properties' => array(
						'confirm' => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Must be true. This deletes imported content.', 'pixelgrade-assistant' ),
						),
					),
					'additionalProperties' => false,
				),
				...
				'execute'     => static function ( $input = array() ) {
					if ( empty( $input['confirm'] ) ) {
						return self::needs_confirmation( __( 'Resetting starter content', 'pixelgrade-assistant' ) );
					}
			...
			'pixelgrade/list-recipes' => array(
				'label'       => __( 'List recipes', 'pixelgrade-assistant' ),
				'description' => __( 'List the source recipes — bundles of layout units that can be applied together — backed by the layout units available to this site. Read-only. Call this to discover the recipe_id pixelgrade/apply-recipe needs. Pass sources to narrow the list to specific hub source ids; omit it for every source. A source that fails to build is reported in warnings and never fails the call.', 'pixelgrade-assistant' ),
				'annotations' => array( 'readonly' => true, 'destructive' => false, 'idempotent' => true ),
				'input_schema' => array(
					'type'       => 'object',
					'properties' => array(
						'sources' => array(
							...
							'description' => __( 'Hub source ids to restrict the listing to. Omit for every source.', 'pixelgrade-assistant' ),
						),
					),
					'additionalProperties' => false,
				),
			...
			'pixelgrade/apply-recipe' => array(
				...
				'input_schema' => array(
					...
					'properties' => array(
						'recipe_id'      => array(
							'type'        => 'string',
							'description' => __( 'The recipe/source id, as listed by pixelgrade/list-recipes.', 'pixelgrade-assistant' ),
						),
						'source_url'     => array(
							'type'        => 'string',
							'description' => __( 'The recipe source\'s SCE REST base URL. Must use https://.', 'pixelgrade-assistant' ),
						),
						'include_look'   => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Also apply the recipe\'s design settings (colors and fonts). These are not reverted if a later step fails.', 'pixelgrade-assistant' ),
						),
						'include_sample' => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Also import the recipe\'s sample content.', 'pixelgrade-assistant' ),
						),
						'confirm'        => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Must be true. This creates content it cannot cleanly reverse.', 'pixelgrade-assistant' ),
						),
					),
				...
				'execute'     => static function ( $input = array() ) {
					$validated = PixelgradeAssistant_Agent_Core::validate_keyed_source(
						...
						isset( $input['source_url'] ) ? $input['source_url'] : '',
						__( 'You need to provide recipe_id and source_url.', 'pixelgrade-assistant' ),
						__( 'source_url must use https://.', 'pixelgrade-assistant' )
					);
					...
					if ( empty( $input['confirm'] ) ) {
						return self::needs_confirmation( __( 'Applying a recipe', 'pixelgrade-assistant' ) );
					}
			...
			'pixelgrade/list-page-patterns' => array(
				'label'       => __( 'List page patterns', 'pixelgrade-assistant' ),
				'description' => __( 'List the page patterns available to this site: complete Page, Post, Project or Product records that can be added one at a time. Read-only. Applying one CREATES A WHOLE NEW RECORD — that is a different action from inserting blocks into the document you are editing, which is a block pattern (see the nova-blocks patterns listing), and from applying an entire site (pixelgrade/import-starter). Call this to discover the slug, unit_type, demo_key and source_url that pixelgrade/import-page-pattern requires; those four values come from here, not from guesswork. Pass sources to narrow the list to specific hub source ids. A source that cannot be read is reported in warnings and never fails the call, so a single unreachable catalog cannot hide the rest.', 'pixelgrade-assistant' ),
				'annotations' => array( 'readonly' => true, 'destructive' => false, 'idempotent' => true ),
				'input_schema' => array(
					'type'       => 'object',
					'properties' => array(
						'sources' => array(
							'type'        => 'array',
							'items'       => array( 'type' => 'string' ),
							'description' => __( 'Hub source ids to restrict the listing to. Omit for every source that offers content records.', 'pixelgrade-assistant' ),
						),
					),
					'additionalProperties' => false,
				),
			...
			'pixelgrade/import-page-pattern' => array(
				...
				'input_schema' => array(
					...
					'properties' => array(
						'slug'       => array(
							'type'        => 'string',
							'description' => __( 'The record\'s source slug, as listed by pixelgrade/list-page-patterns.', 'pixelgrade-assistant' ),
						),
						'demo_key'   => array(
							'type'        => 'string',
							'description' => __( 'The source\'s demo key, as listed by pixelgrade/list-page-patterns.', 'pixelgrade-assistant' ),
						),
						'source_url' => array(
							'type'        => 'string',
							'description' => __( 'The source\'s SCE REST base URL. Must use https://.', 'pixelgrade-assistant' ),
						),
						'unit_type'  => array(
							'type'        => 'string',
							'default'     => 'page',
							'description' => __( 'The record\'s post type: page, post, portfolio or product. Defaults to page.', 'pixelgrade-assistant' ),
						),
						'confirm'    => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Must be true. This creates content and media, and replaces an already-applied copy of the same pattern.', 'pixelgrade-assistant' ),
						),
					),
				...
				'execute'     => static function ( $input = array() ) {
					...
					$selector = PixelgradeAssistant_Agent_Core::validate_page_pattern_selector(
						...
						__( 'You need to provide the page pattern slug.', 'pixelgrade-assistant' ),
						/* translators: 1: the given record type, 2: the accepted types. */
						__( 'unit_type must be one of %2$s; got "%1$s".', 'pixelgrade-assistant' )
					);
					...
					if ( empty( $input['confirm'] ) ) {
						return self::needs_confirmation( __( 'Importing a page pattern', 'pixelgrade-assistant' ) );
					}
