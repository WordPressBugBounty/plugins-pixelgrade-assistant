...
class PixelgradeAssistant_Agent_Core {
	...
	public static function assistant_unavailable() {
		return self::result(
			...
			__( 'Pixelgrade Assistant core modules are not loaded.', 'pixelgrade-assistant' ),
			array(),
			array(),
			1
		);
	...
	public static function list_starters( $params = array() ) {
		...
		if ( false === $remote_config ) {
			return self::result(
				false,
				'hub_fetch_failed',
				__( 'Could not reach the Pixelgrade hub to fetch starter descriptors.', 'pixelgrade-assistant' ),
				array(),
				array(),
				1,
		...
		return self::result(
			...
			sprintf(
				...
				_n( '%d starter site available.', '%d starter sites available.', count( $starters ), 'pixelgrade-assistant' ),
				count( $starters )
			),
	...
	public static function import_starter( $params ) {
		...
		if ( 'success' === $code ) {
			return self::result(
				true,
				'ok',
				'' !== $message ? $message : __( 'Starter content imported.', 'pixelgrade-assistant' ),
				$data,
				array(),
				0
			);
	...
	public static function reset_starter_content() {
		...
		if ( $posts_missing > 0 ) {
			return self::result(
				...
				sprintf(
					...
					__( 'Starter content reset, but %d journaled post(s) were already missing.', 'pixelgrade-assistant' ),
					$posts_missing
				),
				$summary,
				array(
					array(
						'code'    => 'posts_missing',
						'message' => __( 'Some journaled posts were already missing and could not be deleted.', 'pixelgrade-assistant' ),
					),
				),
				2
			);
		...
		return self::result( true, 'ok', __( 'Starter content was reset.', 'pixelgrade-assistant' ), $summary, array(), 0 );
	}
	...
	public static function list_page_patterns( $params = array() ) {
		...
		return self::result(
			...
			sprintf(
				...
				_n( '%d page pattern available.', '%d page patterns available.', count( $units ), 'pixelgrade-assistant' ),
				count( $units )
			),
			$data,
			$warnings,
	...
	public static function import_page_pattern( $params ) {
		...
		if ( 'success' === $code ) {
			return self::result(
				true,
				'ok',
				'' !== $message ? $message : __( 'Page pattern imported.', 'pixelgrade-assistant' ),
				$data,
				array(),
				0
			);
	...
	public static function list_recipes( $params = array() ) {
		...
		return self::result(
			...
			sprintf(
				...
				_n( '%d recipe available.', '%d recipes available.', count( $recipes ), 'pixelgrade-assistant' ),
				count( $recipes )
			),
			$data,
			$warnings,
	...
	public static function apply_recipe( $params ) {
		...
		if ( 'success' === $code ) {
			return self::result(
				true,
				'ok',
				'' !== $message ? $message : __( 'Recipe applied.', 'pixelgrade-assistant' ),
				$data,
				array(),
				0
			);
		...
		if ( ! empty( $options['include_look'] ) ) {
			$warnings[] = array(
				...
				'message' => __( 'This recipe requested the look. The failure may have happened after design settings (colors/fonts) were already written; they are not automatically reverted. Check the site\'s Style Manager settings before retrying.', 'pixelgrade-assistant' ),
			);
		}
