...
class PixelgradeAssistant_MCP_Server {
	...
	public static function redact_tool_error( $result, $args = array(), $tool_name = '', $mcp_tool = null, $server = null ) {
		...
		return new WP_Error(
			...
			__( 'The ability failed unexpectedly. The details were written to the site error log.', 'pixelgrade-assistant' )
		);
	}
	...
	public static function create_server( $adapter ) {
		try {
			$adapter->create_server(
				...
				self::SERVER_ROUTE,
				__( 'Pixelgrade', 'pixelgrade-assistant' ),
				__( 'Read and shape a Pixelgrade site: its design system, block inventory, license status and starter content.', 'pixelgrade-assistant' ),
				self::SERVER_VERSION,
	...
	public static function within_request_limits( $request = null ) {
		...
		if ( $max_bytes > 0 && strlen( $body ) > $max_bytes ) {
			return new WP_Error(
				...
				__( 'The request body is larger than this server accepts.', 'pixelgrade-assistant' ),
				array( 'status' => 413 )
			);
		}
			...
			if ( is_array( $decoded ) && array_key_exists( 0, $decoded ) && count( $decoded ) > $max_batch ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'This server accepts at most %d batched messages per request.', 'pixelgrade-assistant' ),
						$max_batch
					),
					array( 'status' => 413 )
				);
