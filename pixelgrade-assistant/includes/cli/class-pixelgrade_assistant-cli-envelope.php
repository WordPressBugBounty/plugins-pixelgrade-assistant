...
class PixelgradeAssistant_CLI_Envelope {
	...
	public static function require_capability( $capability, $assoc_args = array() ) {
		...
		self::emit(
			...
			sprintf(
				...
				__( 'This command requires the "%s" capability. WP-CLI runs as no user unless you pass --user=<username> for one who has it.', 'pixelgrade-assistant' ),
				$capability
			),
			array(),
			array(),
	...
	public static function require_yes_or_halt( $assoc_args, $command_example ) {
		if ( self::confirmed( $assoc_args, __( 'This is a destructive operation. Proceed?', 'pixelgrade-assistant' ) ) ) {
			return;
		}
		...
		self::emit(
			...
			sprintf(
				...
				__( 'This is a destructive operation; pass --yes to confirm. Example: %s', 'pixelgrade-assistant' ),
				$command_example
			),
			array(),
			array(),
